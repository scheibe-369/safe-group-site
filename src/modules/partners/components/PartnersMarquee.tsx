          ...
          {TRACK.map((partner, index) => (
            <div key={`${partner.id}-${index}`} className="group flex h-24 shrink-0 flex-col items-center justify-center gap-3 px-10">
              <div
                ...
                className="flex h-10 items-center justify-center text-white/40 [filter:grayscale(1)] transition-all duration-[400ms] [transition-timing-function:cubic-bezier(0.22,1,0.36,1)] group-hover:-translate-y-2 group-hover:text-[var(--brand)] group-hover:[filter:grayscale(0)_drop-shadow(0_0_14px_var(--glow))]"
              >
                <BrandMark partner={partner} height={32} />
              </div>
