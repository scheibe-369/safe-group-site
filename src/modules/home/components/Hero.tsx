...
export async function Hero() {
  ...
  return (
    <section className={`${styles.hero} relative flex min-h-[760px] items-end overflow-hidden bg-black pb-16 pt-28 sm:min-h-[820px] lg:h-dvh lg:min-h-[760px] lg:items-start lg:pb-0 lg:pt-[clamp(11rem,22vh,16rem)]`}>
      ...
      <div className="safe-edge relative z-10">
        <div className={`${styles.content} max-w-2xl xl:max-w-3xl 2xl:max-w-4xl`}>
          ...
          <span aria-hidden data-intro-reveal="line" data-intro-step="1" className={`${styles.rule} mt-6 block h-[3px] w-[4.5rem] bg-[var(--safe-red)]`} />
          <h1 className={`${styles.title} mt-7 text-[clamp(2.5rem,5vw,4.75rem)] font-semibold uppercase leading-[1.14] tracking-[-.02em] text-white`}>
          ...
          <div className={`${styles.lead} intro-mask mt-6 max-w-xl`} data-intro-step="3">
            <p className="text-base leading-7 text-white/75 sm:text-lg">{hero.lead}</p>
          </div>
          <div className={`${styles.areas} intro-mask mt-4`} data-intro-step="4">
          ...
          <div className={`${styles.actions} mt-7 flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:gap-8`}>
