...
- `components/SiteIntro.tsx`: le o idioma no servidor e desce a copy ao `IntroCurtain.tsx`, que traz o script inline que escreve `data-intro="pending"` e `data-intro-at` no `<html>` antes da primeira pintura (so sem movimento reduzido), e a cortina em si. Monta-se como primeiro filho do `<body>` no layout.
- `data/timeline.ts`: os instantes da coreografia, em segundos. Unico sitio onde se mexe no ritmo.
- `data/content.pt-PT.ts`: a linha de estado e as quatro leituras do contador, na lingua de origem. `data/content.ts` junta os cinco idiomas.
