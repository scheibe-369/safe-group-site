...
const TAG = [...introContent.tag];
const TAG_OUT = stagger(TAG.length, tagOut, spread.tagOut, 7);

/**
 * A primeira leitura acende-se ao lado da marca, no mesmo grupo baralhado, e a
 * ultima apaga-se com ela. Nos dois casos a marca e o alvo a seguir ao ultimo
 * caractere, por isso o grupo leva mais um.
 */
const FIRST = [...introContent.steps[0]];
const LAST = [...introContent.steps[introContent.steps.length - 1]];
...
const STEPS = introContent.steps.map((label, step) => {
  const chars = [...label];
  const first = step === 0;
  const last = step === introContent.steps.length - 1;
  const rollIn = first ? OPEN : stagger(chars.length, enter[step], spread.roll);
  const rollOut = last ? CLOSE : stagger(chars.length, leave[step], spread.roll);
        ...
        <div className="site-intro__wrap">
          <div className="site-intro__item">
            <p className="site-intro__tag">
              {TAG.map((char, index) => (
                <span key={index} className="site-intro__leave" style={seconds({ out: TAG_OUT[index] })}>
                  {char === " " ? " " : char}
                </span>
              ))}
            </p>
