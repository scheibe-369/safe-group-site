      ...
      <div data-intro-reveal="bar" className="safe-edge pointer-events-none relative z-10 flex min-h-[6em] items-center justify-between gap-[1.5em]">
        ...
        <nav aria-label="Navegação principal" className="site-nav__links hidden items-center gap-[1.5em] lg:flex xl:gap-[2em]">
          {content.barLinks.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} onClick={() => navigate(link.href)} aria-current={active ? (link.href.includes("#") ? "location" : "page") : undefined} className={`nav-mask-trigger pointer-events-auto inline-flex min-h-11 items-center text-[0.75em] font-medium uppercase tracking-[.12em] ${active ? "text-white" : "text-white/80"}`}>
